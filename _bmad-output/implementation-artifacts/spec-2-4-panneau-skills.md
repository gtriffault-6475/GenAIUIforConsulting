...
**Reviewer Gate complet, effectué rétroactivement (2026-09-16)** après reprise de session : les trois lentilles parallèles habituelles (adversariale, cas limites, gaps de vérification) ont été exécutées contre le diff committé (`af177c4..5490e0a`), comblant la revue allégée notée ci-dessus. Voir le Review Triage Log complet ci-dessous.
...
|---|---|---|---|---|
| 1 | Revue allégée initiale (une passe directe de l'orchestrateur, pas trois lentilles parallèles) suite à une demande de pause utilisateur en cours de story. | — | No action (process documenté) | `tsc`/`next build` revérifiés propres à l'époque ; complété depuis par le Reviewer Gate complet ci-dessous. |
| 2 | Le libellé de section "Skills chargées" précède le point d'entrée "Ajouter une skill" dans le DOM, ce qui pourrait sembler contredire la lecture littérale du texte gelé de la spec ("reste le premier élément visible du panneau"). Trouvé par la revue cas limites. | Low | No action (false) | Même ordre déjà établi et accepté sans réserve pour `ConversationList.tsx` (libellé "Conversations" avant le bouton "Nouvelle conversation", Story 2.2) — la garantie fonctionnelle réelle ("jamais remplacé par un état d'erreur") est respectée ; "premier" s'interprète comme "avant toute branche liste-vide/liste-pleine/erreur", cohérent avec le pattern déjà en place. |
| 3 | `seedFixturesIfEmpty` (`actions/skill.ts`) ne vérifie que "zéro ligne" pour décider de seeder — même lacune structurelle déjà documentée pour `actions/conversation.ts` (Story 2.2), non re-tracée ici. Trouvé par la revue cas limites. | Low | Defer | Logué dans `deferred-work.md`, miroir exact de l'entrée Story 2.2 — inerte aujourd'hui (aucun chemin n'écrit une ligne `PROJECT_SKILL` hors de ce seeder), à revisiter si un mécanisme d'ajout de skill est un jour construit. |
| 4 | Convention systémique pré-existante (pas spécifique à cette story) : les messages d'erreur français retournés par `actions/skill.ts` (comme tous les autres fichiers `actions/`) sont techniquement dans `actions/`, ce qui lu littéralement contredit la règle CONVENTIONS.md "chaînes visibles par l'utilisateur... jamais dans actions/". Trouvé par la revue adversariale. | Low | No action | Pattern identique dans `actions/project.ts`, `actions/document.ts`, `actions/conversation.ts`, `actions/mattermost.ts` — pas une régression de cette story ; `SkillsPanel.tsx` n'affiche même pas cette chaîne (message de repli codé en dur côté composant, comme `ConversationList.tsx`). Hors scope d'une story ciblée. |
| 5 | Clic-through réel en navigateur (overlay "Ajouter une skill", exclusivité AD-8 face à `ProjectSelector`/`AddDocumentForm`) non vérifiable dans cet environnement non supervisé — confirmé indépendamment par les trois revues (chacune a tenté et échoué à démarrer un navigateur/serveur de preview). | — (limite d'environnement) | No action | Mécanisme `OverlayProvider` (état `openOverlayId` unique, jamais fusionné) vérifié statiquement comme garantissant structurellement l'exclusivité — même mécanisme déjà prouvé en direct par clic pour `ProjectSelector`/`AddDocumentForm` lors des Stories 1.2/1.4 ; `SkillsPanel` n'introduit aucune logique d'overlay propre, seulement un `OVERLAY_ID` distinct. |
| — | AD-2, AD-4, AD-8, seed idempotent/race-safe (mécanisme transaction synchrone tracé jusqu'au driver `node:sqlite`), chemin skillKey orphelin, formes de données, `--color-ai-accent` réservé à l'icône, absence de mécanisme d'ajout fonctionnel simulé | — | No action | Vérifiés indépendamment et en direct (seed testé sur les 3 projets, chemin d'erreur skillKey forcé et reverté, `tsc`/`next build` revérifiés) par les trois revues — corrects tels qu'implémentés. |
...
**Reviewer Gate complet — trois revues parallèles indépendantes (2026-09-16) :**
- `npx tsc --noEmit`/`npx next build --turbopack` revérifiés propres par la revue gaps de vérification, indépendamment.
- Seed distinct par projet (`proj-acme-rfp` → `references`+`rfp-drafting`, `proj-audit-mission` → `references`+`mission-scoping`), idempotence sur double appel, et projet inconnu → liste vide sans seed, tous re-testés en direct contre `db/local.db` (sauvegardé puis restauré, md5 identique avant/après).
- Chemin `skillKey` orphelin forcé en direct (ligne `PROJECT_SKILL` avec clé inexistante insérée puis retirée) : log `console.error` confirmé, ligne ignorée, reste de la liste servi normalement, aucun crash.
- Mécanisme d'exclusivité `OverlayProvider` (AD-8) confirmé structurellement garanti par lecture du code (`openOverlayId` unique, jamais fusionné) par les trois revues ; le clic-through réel en navigateur reste non vérifiable dans cet environnement non supervisé (confirmé indépendamment par les trois tentatives échouées).
- Aucun correctif de code nécessaire à l'issue du Reviewer Gate complet — voir Review Triage Log.
